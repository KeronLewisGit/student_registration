...
<html lang="en">
    ...
    <body>
        ...
        {
            return strnatcmp($recordA["Student Name"], $recordB["Student Name"]);
        }
        ...
        foreach ($records as $record){
            if ($record['Form 1 Class'] == '1B'){
                $form1B[]= "'".$record["Student Name"]."'";
            }
        }
        foreach ($records as $record){
            if ($record['Form 1 Class'] == '1C'){
                $form1C[]= "'".$record["Student Name"]."'";
            }
        }
        foreach ($records as $record){
            if ($record['Form 1 Class'] == '1D'){
                $form1D[]= "'".$record["Student Name"]."'";
            }
        }
        foreach ($records as $record){
            if ($record['Form 1 Class'] == '1E'){
                $form1E[]= "'".$record["Student Name"]."'"; 
            }
        }
        $form1Array= array();
        foreach ($form1A as $A){
            array_push($form1Array, $A);
        }
        $form1Brray= array();
        foreach ($form1B as $B){
            array_push($form1Brray, $B);
        }
        $form1Crray= array();
        foreach ($form1C as $C){
            array_push($form1Crray, $C);
        }
        $form1Drray= array();
        foreach ($form1D as $D){
            array_push($form1Drray, $D);
        }
        $form1Erray= array();
        foreach ($form1E as $E){
            array_push($form1Erray, $E);
        }
            ...
            <form class="row g-3">
                    <div class=" col-md-6">
                        ...
                        <select name="student_class" id="student_class" class="form-select">
                            <option value= "0" selected>Student Class</option>
        ...
        foreach ($records as $record) {
            ...
            echo '<div class="'.str_replace(" ", "",$record['Student Name'])." ".$record['Form 1 Class'].' pagestyle">';
    ...
    <script>
        ...
        var students = {
            <?php
                echo "'1A': [".implode(", ", $form1Array).'],';
                echo "'1B': [".implode(", ", $form1Brray).'],';
                echo "'1C': [".implode(", ", $form1Crray).'],';
                echo "'1D': [".implode(", ", $form1Drray).'],';
                echo "'1E': [".implode(", ", $form1Erray).']';
            ?> 
        }
        ...
        var $student_classes =$('#student_class');
            ...
            var html = $.map(lens, function (lcn){
                return '<option value="' + (lcn.replaceAll(' ', '')) + '">' + lcn + '</option>'; 
            }).join('');
            $students.html(html);
            ...
            $("#student_name").prop('selectedIndex',0);
            if(optionValue){
